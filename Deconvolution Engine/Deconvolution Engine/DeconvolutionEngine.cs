...
namespace Deconvolution_Engine
{
    public partial class Form1 : Form
    {
        ...
        private void prepareDatabase()
        {
            if (currIndex < allRawFileNames.Count)
            {
                ...
                var newThread = new Thread(() => dbPrep.PrepDatabase(currRawFileString, outputTextBox.Text));
                newThread.Start();
            }
            else
            {
        ...
        private void extractFeatures()
        {
            if (currIndex < allRawFileNames.Count)
            {
                ...
                currRawFile.Open();
                var path = outputTextBox.Text + "\\" + currRawFile.Name + "_ExtractedFeatures.sqlite";
                ...
                if (!SQLiteIOMethods.IsExtractionDone(conn))
                {
                    ...
                    var newThread = new Thread(() => currFDM.ExtractFeaturesAndPushToDatabase(currRawFile, conn));
                    newThread.Start();
                }
                else
                {
        ...
        private void GroupFeatures()
        {
            if (currIndex < allRawFileNames.Count)
            {
                ...
                if (!SQLiteIOMethods.IsGroupingDone(conn))
                {
                    ...
                    var newThread = new Thread(() => currFDM.GroupFeatures(conn));
                    newThread.Start();
                }
                else
                {
